...
local Goofer={
	...
	"vote for miku in the election";
}

local gameName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name

local playerToMonitorId = 7175945807
local function onPlayerJoined(player)
    if player.UserId == playerToMonitorId then
        DoNotif(player.Name.. " has joined the game!" ,5)
end
end
...
cmd.add({"vulnrabilitytest","vulntest"},{"vulnrabilitytest (vulntest)","Test if your executor is vulnrable"},function()
...
cmd.add({"byfron"},{"byfron", "fake byfron kick"},function()
    loadstring(game:HttpGet("https://pastebin.com/raw/nUE1qy9Q"))();
    end)
...
cmd.add({"kick"},{"kick <reason>","Kicks you fron the game"},function(...)
	local A_1=""
	local table={...}
	for i,v in pairs(table) do
		if i~=1 then
	...
	local A_2="All"
	game.Players.LocalPlayer:Kick(A_1,A_2)
end)
			...
			elseif v:IsA("Model") or v:IsA("Folder") then
				for _,j in ipairs(v:GetDescendants()) do
					if j:IsA("BasePart") then
						wait(0.1)
						game.Workspace.CurrentCamera.CameraSubject=j
						break
					end
				end
			end
...
ImageButton.Image="rbxassetid://18877381572"
